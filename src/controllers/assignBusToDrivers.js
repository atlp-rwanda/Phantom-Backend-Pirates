...
class Buses {
  static async assign(req, res) {
    helper
      ...
      .then(async (rolepermission) => {
        if (rolepermission) {
          const emptyEmployee = req.t(
            'assigning_bus_driver_message.missingEmployee'
          );
          const employeeNotbeingADriver = req.t(
            'assigning_bus_driver_messages.employeeNotBeingADriver'
          );
          const driverHasbus = req.t(
            'assigning_bus_driver_messages.driverExists'
          );
          const busNotFound = req.t(
            'assigning_bus_driver_messages.busNotFound'
          );
          const busHasADriver = req.t(
            'assigning_bus_driver_messages.busHasADriver'
          );

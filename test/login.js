  ...
  describe('/users/login', () => {
    ...
    it('A non-registered user shouldnot be able to login/Kinyarwanda Language', (done) => {
      ...
      chai
        ...
        .end((err, res) => {
          res.should.have.status(400)
          res.body.error.should.be.equal("Imeri yanditswe ntabwo tuyifite! Nyamuneka banza wiyandikishe")
          done()
        })
    })
    it('A non-registered user shouldnot be able to login/French Language', (done) => {
      ...
      chai
        ...
        .end((err, res) => {
          res.should.have.status(400)
          res.body.error.should.be.equal("L'e-mail n'est pas enregistré ! Veuillez d'abord vous inscrire")
          done()
        })
    })
    it('A registered user with wrong password logins shouldnot be able to login/Default Language', (done) => {
      ...
      chai
        ...
        .end((err, res) => {
          ...
          res.body.error.should.be.equal("The email or passwords entered is wrong")
          done()
        })
    })
    it('A registered user with wrong password logins shouldnot be able to login/Kinyarwanda Language', (done) => {
      ...
      chai
        ...
        .end((err, res) => {
          res.should.have.status(401)
          res.body.error.should.be.equal("Imeri cyangwa ijambo ryibanga ryinjiye sibyo")
          done()
        })
    })
